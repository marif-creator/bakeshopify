<template>
  <div class="space-y-6">
      ...
      <div class="overflow-x-auto">
        <UTable :rows="filteredProducts" :columns="(columns as any)">
          ...
          <template #stock-data="{ row }">
            <UBadge :color="(row as any).stock > 10 ? 'success' : (row as any).stock > 0 ? 'warning' : 'error'" variant="subtle">
    ...
    <UModal v-model="showProductDetailsModal" title="Product Details" class="max-w-3xl">
    ...
    <UModal v-model="showDeleteModal" title="Confirm Deletion" class="max-w-md">
      <div class="p-6 space-y-4">
        <p class="text-gray-700">
          Are you sure you want to delete "{{ (productToDelete as any)?.title }}"? This action cannot be undone.
        </p>
        <div class="flex justify-end gap-2">
          <UButton @click="showDeleteModal = false" variant="outline">
            Cancel
          </UButton>
          <UButton @click="confirmDelete" color="error">
            Delete
          </UButton>
        </div>
      </div>
...
// Define page meta for authentication and layout
definePageMeta({
  auth: true,
  layout: 'seller'
})
